...
const ListWrapper = () => {
  const accountingPurchase: BasicTableState = useSelector((state: ReduxState) => state.accountingPurchase);
  const { dataList, helpers } = useBasicTable("/accounting-purchase", accountingPurchase, actions);

  useEffect(() => {
    if (accountingPurchase.isFirstTime) {
      helpers.fetchData();
    }
  }, []);
  ...
  return (
    <BasicTable
      {...helpers}
      headerAddButton
      columnsList={Columns(accountingPurchase, helpers)}
      dataList={dataList}
    >
